...
# empty-i ⇄ empty-j via `import {} from`, which still evaluates the module
for f in value-a value-b mixed-g mixed-h empty-i empty-j; do
...
for f in type-c type-d texp-k texp-l texp-m texp-n lazy-e lazy-f; do
